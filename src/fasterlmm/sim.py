...
Generates a (N, M) genotype matrix + a phenotype with a known h2 from a small number of causal SNPs.  Mostly
handy for trying out the scan + perm threshold on something where the truth is known
"""
...
def sim_gwas(N: int = 100, M: int = 500, *, h2: float = 0.5, n_causal: int = 10,
             seed: int = 19930909) -> dict:
    """
    Quantitative GWAS sim
    Z is iid biallelic dosages (binomial(2, 0.5)).
    Pheno y has heritability h2 driven by n_causal randomly-picked SNPs with iid normal effects, gaussian
    residual makes up the rest
