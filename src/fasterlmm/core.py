...
class ScanResult:
    ...
    max_F is (P,) and covers every column, reals and perms both, since the perm columns only ever keep this
    running genome max and never the full per-SNP detail
...
def _eigh_with_cpu_fallback(K: Tensor) -> tuple[Tensor, Tensor]:
    ...
    cuSOLVER's eigh workspace is 3-8x the input.  At large N the workspace OOMs even when K, U, s themselves
    fit on the GPU.  Shipping K to cpu, calling lapack eigh, and bringing s and U back is slow but it's the
    only thing that works at the edge
    mps has no symmetric-eigh kernel at all, so a metal K detours through cpu lapack every time -- it's once
    per chromosome so the round-trip barely registers next to the scan
    """
...
def _pos_floor(dtype: torch.dtype) -> float:
    ...
    1e-300 is fine in float64 but underflows clean to 0 in float32, and a 0 floor turns a constant column's
    denominator into a 0/0 nan, so the float32 path gets a representable floor insted
    """
...
def eigendecompose(K: Tensor, jitter: float = 1e-6) -> tuple[Tensor, Tensor]:
    ...
    fastlmm does SVD on G then squares the singular values when k<N (faster), with an eigh fallback if SVD
    blows up
    Going straight to torch.linalg.eigh on K beacuse the cases that matter are full-rank (k>=N), with a tiny
    diagonal jitter for safety on near-singular K and a cpu lapack fallback if cuSOLVER's workspace OOMs
    """
    N = K.shape[0]
...
def rotate(K: Tensor, X: Tensor, Y: Tensor) -> Spectrum:
    ...
    fastlmm splits these across setX (line 70 -> self.UX) and setY (line 94 -> self.Uy), each lazy on prior
    eigendecomp state.  Bundling them here makes the data dependency explicit -> you cant call rotate before
    you have K
    """
...
def nLLeval(delta: float | Tensor, s: Tensor, X_rot: Tensor, y_rot: Tensor) -> Tensor:
    """
    PORT IS DONE!
    ...
    Vectorising the loss across a (G grid x P pheno) tensor, the per-pheno call is fine while there's no scan
    yet
    """
    N, _ = X_rot.shape
    ...
    # residual SS in V-metric, identity avoids forming y - X beta
    ...
    # -2 loglik, +1 from profiling sigma2_g back in, sum log(Sd) is log det V
    return N * (log2pi + sigma2_g.log() + 1.0) + Sd.log().sum()


def _profile_loss(log_delta: Tensor, s: Tensor, X_rot: Tensor, Y_rot: Tensor, *,
                  reml: bool = False) -> Tensor:
    """
    PORT IS DONE!
    Vectorised -2*loglik on a (G grid x P pheno) tensor, same algebra as nLLeval just broadcast across both
    axes (lmm.py LMM.nLLeval). ->> Returns shape (G, P)
    reml=False (default) is the ML branch.
    reml=True adds the X-side penalty term log det A and divides by (N - C) instead of N, same as fastlmm's
    REML
    Replaces the per-pheno python loop in fit_delta_grid since perms drag in hundreds of pheno columns and the
    loop was the bottleneck
    """
    N, C = X_rot.shape
...
def _profile_loss_per_pheno(log_delta: Tensor, s: Tensor, X_rot: Tensor, Y_rot: Tensor, *,
                            reml: bool = False) -> Tensor:
    """
    Per-pheno _profile_loss for the golden-section refinment: one log_delta value per pheno, returns (P,)
    Same algebra as _profile_loss but the grid axis is replaced by the pheno axis, so each pheno evaluates
    only its own log delta.  Way cheaper than rebuilding a (G, P) grid just to read one column per pheno
    """
    N, C = X_rot.shape
...
def fit_delta_grid(spectrum: Spectrum, *, n_grid: int = 64, log_delta_min: float = -10.0,
                   log_delta_max: float = 10.0, refine: bool = True, reml: bool = False) -> Tensor:
    """
    Multi-pheno log-delta fit: 64-point grid then optional golden-section refinment, returns (P,)
    fastlmm's findH2 uses grid + Brent via minimize1D (fastlmm/util/mingrid.py).  Golden-section here gets to
    ~1e-10 in log delta after 50 iterations.  refine=False just returns the grid argmin if speed matters more
    than precision
    reml=False (default) matches the ML branch in _profile_loss, reml=True uses the same REML loss in both the
    grid and the refinment
    """
    ...
    # vectorised golden-section refinment on [grid[idx-1], grid[idx+1]] per pheno.  50 iters gets to ~1e-10 in
    # log delta wich is way past what the scan needs
...
def _auto_tile(N: int, C: int, M: int, P: int, device: torch.device) -> tuple[int, int]:
    """
    Pick the pheno / snp tile sizes for snp_wald_scan from the GPU memory free right now
    The inner loop's working set is roughly 8 * (Pc*N*C + N*Mc + (2*C + T)*Pc*Mc) bytes for a pheno tile Pc
    and a snp tile Mc, with T counting the dozen-ish (Pc, Mc) temporaries it keeps alive at once.  Budgeting
    half the free VRAM for that leaves slack for the cholesky workspace and allocator fragmentation
    one budget, two tiles fall out of it -- no machine-specific hand tuning.  cpu / mps have no mem_get_info
    so they fall back to a fixed 256 / 4096
    """
    if device.type != "cuda":
...
def snp_wald_scan(spectrum: Spectrum, log_delta: Tensor, S_rot: Tensor, *, snp_chunk: int | None = None,
                  pheno_chunk: int | None = None, n_real: int | None = None) -> ScanResult:
    """
    PORT IS DONE!
    Multi-pheno per-(SNP, pheno) Wald F-stat at the per-pheno fitted log delta (lmm.py LMM.getPosteriorWeights
    + the per-SNP loop in _internal_single_snp around line 1300)
    Generalises the single-pheno scan to a (P,) log_delta vector, every per-SNP term broadcats a P axis.
    snp_chunk / pheno_chunk tile both axis, and left None they get auto-sized by _auto_tile to fill about half
    the free VRAM -- no hand tuning per machine
    The per-pheno null fit (cholesky_ex on A, beta_x, residual r, rWr) gets computed once per pheno chunk and
    reused for each SNP chunk insde it
    n_real splits the pheno axis: the first n_real columns are real phenos and keep their full per-SNP detail
    (F, beta, SE, SnpFractVarExpl), the rest are permutation columns and only feed a per-column running max.
    That way a perm batch of thousands of columns never materialises an (M, P) tensor -- the threshold only
    ever wants the per-perm genome max anyway.  Returns a ScanResult
    """
    s, X_rot, Y_rot = spectrum.s, spectrum.X_rot, spectrum.Y_rot
    N, C = X_rot.shape
    P = Y_rot.shape[1]
    ...
    return ScanResult(f = f_real, beta = beta_real, se = se_real, sfve = sfve_real, nullh2 = nullh2, max_F = max_F)
...
def loco_scan(Z: Tensor, X: Tensor, Y: Tensor, chrom: np.ndarray, *, n_real: int | None = None,
    ...
    (_internal_single_snp_LocoGwas around line 1100). Y is (N, P), returns a ScanResult
    For each chromosome c the kinship is rebuilt without c, refit per-pheno log delta against K_loco,
    snp_wald_scan on c-only SNPs.
    Looping chromosomes here is fewer moving parts than fastlmm's LocoGwas + SnpReader subset and lets the GPU
    chew through the per-chrom eigh wihtout going back to pysnptools
    The grm + eigendecomposition + rotations are genotype-only, so they cost the same whether Y carries one
    pheno or a whole batch of them.  That's the whole point of feeding loco_scan a wide Y -- one pheno per
    call repays the 16 eighs every pheno, a batch of them repays it once
    Each chromosome's SNPs carry that chromosome's null h2 (fastlmm's Nullh2), since under LOCO the kinship
    the SNP is tested against drops its own chromosome.  n_real flows to snp_wald_scan: leading columns keep
...
class CompatSpectrum:
    ...
    fastlmm's projection formulation (lmm_cov.py lines 117-131): form K_ = (I - PX)(K + I)(I - PX), eigh that,
    drop the first D = ncols(X) eigenvectors and subtract the 1 back off the kept eigenvalues
    The drop-D cut is exact when rank(X) == D but throws away D - rank(X) real dimensions when X is
    rank-deficient.  Reproducing that quirk on purpose, matching it is the whole point of the parity port
...
def fastlmm_compat_rotate(K: Tensor, X: Tensor, Y: Tensor) -> CompatSpectrum:
    """
    PORT IS DONE!
    fastlmm's projection-based rotation (lmm_cov.py lines 117-131)
    Projects K onto the orthogonal complement of X with an OLS hat, eigendecomposes, drops the first D =
    ncols(X) eigenvectors.  Y gets the same OLS regress-out and lands projected onto the kept basis
...
def _profile_loss_compat(log_delta: Tensor, s: Tensor, UY: Tensor) -> Tensor:
    """
    PORT IS DONE!
    Vectorised -2*loglik on a (G grid x P pheno) tensor for the compat path (lmm_cov.py LMM._nLLeval, ML
    branch)
    X is already regressed out in the projected basis so it never appears, the loss is just the variance term
    plus log det V.  ML only, fastlmm's findH2 has no REML branch.  Returns shape (G, P)
    """
    Neff = s.shape[0]
...
def _profile_loss_compat_per_pheno(log_delta: Tensor, s: Tensor, UY: Tensor) -> Tensor:
    """
...
def fit_delta_grid_compat(spec: CompatSpectrum, *, n_grid: int = 64, log_delta_min: float = -10.0,
                          log_delta_max: float = 10.0, refine: bool = True) -> Tensor:
    """
    Multi-pheno log-delta fit for the compat path: 64-point grid then golden-section refinment, returns (P,)
    Same search as fit_delta_grid but on the compat profile loss, wich carries no covariate term since X is
    already projected out.  ML only, no reml flag because fastlmm's findH2 doesnt have one
    """
    s, UY = spec.s, spec.UY
    grid = torch.linspace(log_delta_min, log_delta_max, n_grid, dtype = s.dtype, device = s.device)
    ...
    # vectorised golden-section refinment on [grid[idx-1], grid[idx+1]] per pheno, 50 iters lands well past
    # what the scan needs
    lo_idx = torch.clamp(idx - 1, 0, n_grid - 1)
    hi_idx = torch.clamp(idx + 1, 0, n_grid - 1)
    a = grid[lo_idx].clone()
...
def snp_wald_scan_compat(spec: CompatSpectrum, log_delta: Tensor, S: Tensor, *, snp_chunk: int = 4096,
                         pheno_chunk: int = 256, n_real: int | None = None) -> ScanResult:
    """
    PORT IS DONE!
    Multi-pheno per-(SNP, pheno) Wald F-stat in the compat (projected) basis (lmm_cov.py around 1050, the
    per-SNP loop in single_snp.py _internal_single_snp near line 1300)
    Test SNPs get the same OLS regress-out against X and projection onto U_eff the phenos already went trough,
    so the whole test runs in the covariate-free reduced space -- no per-grid-point (C, C) cholesky, wich is
    what makes the compat path the fast one
    Per-(SNP, pheno) dof is Neff - 1 = N - D - 1, matching fastlmm's _nLLcore rebinding N := N - linreg.D and
    the F-test in single_snp.py:1415
    n_real splits the pheno axis the usual way: the first n_real columns keep full per-SNP detail (F, beta,
    SE, SnpFractVarExpl), the rest are permutation columns and only feed the per-column running max F.
    ...
    # OLS-regress X out of the test SNPs, then project onto the kept basis -- the same two steps the phenos
    # took in fastlmm_compat_rotate
    S_r = S - X @ (Xpinv @ S)  # (N, M)
    ...
    for p_start in range(0, P, pheno_chunk):
        ...
        for m_start in range(0, M, snp_chunk):
            ...
            if real_end > p_start:
                ...
                beta_real[m_start:m_end, p_start:real_end] = beta[:rsl].T
                se_real[m_start:m_end, p_start:real_end] = var_beta[:rsl].sqrt().T
                # SnpFractVarExpl = sqrt(sWy² / (denom * YKY)), lmm_cov.py:1068. compat X is already regressed
                # out so rWr is YKY directly
...
def loco_scan_compat(Z: Tensor, X: Tensor, Y: Tensor, chrom: np.ndarray, *, n_real: int | None = None,
    ...
    (_internal_single_snp_LocoGwas around line 1100). Y is (N, P), returns a ScanResult
    Same shape contract as loco_scan, but every chromosome rotates trough fastlmm_compat_rotate so the
    covariates get the OLS regress-out and the D-eigenvector drop that match fastlmm bit-for-bit
    For each chromosome c the kinship is rebuilt without c, refit per-pheno log delta against K_loco,
    snp_wald_scan_compat on c-only SNPs.  Each chromosome's SNPs carry that chromosome's null h2.  n_real
