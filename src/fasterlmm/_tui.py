...
A pastel palette mirroring the matplotlib figures (sky / peach / mint / dusty rose), one progress-bar helper
and a per-shard colour cycle.  Panel bodies stay flat Text everywhere, Rich Live can't measure a Group nested
inside a Panel and renders it below instead of in place
"""
...
PALETTE: dict[str, str] = {"primary": "rgb(126,184,212)",  # pastel sky, the fasterlmm core colour
...
def progress_bar(done: float, total: float, width: int = 40, *, style: str = "primary") -> Text:
    """
    Flat unicode bar tinted from the palette
    An empty or unknown total renders all light blocks; a bar that reaches the end flips to the success colour
    so a finished shard reads diferent at a glance
    """
