...
When the kinship is built from k pruned markers and k < N, K = G Gᵀ has rank k, so decomposing the N-by-k
factor G beats eighing the N-by-N kernel -- O(N k²) and O(N k) insted of O(N³) and O(N²)
This is fastlmm's own k<N branch (lmm_cov.py setSU_fromG), not a new model.  The full-rank compat path in
core.py forms K and projects it, this one never forms K at all
The N - k null directions of K never get an eigenvector, they only ever see delta, so the likelihood carries
them analyticaly through the off-rank residual UUA = A - U Uᵀ A and an (Neff - k) log delta term in the log
det
"""
...
class LowRankBasis:
    """
    Genotype-only part of the low-rank decomposition, pheno-independent so it's computed once per chromosone
    and resued across every pheno chunk
...
class LowRankSpectrum:
    ...
    Mirrors CompatSpectrum but for the k < N case, so it also carries the off-rank residal of Y and the true
    reduced sample size Neff = N - D that the rank k no longer equals
...
def lowrank_basis(G: Tensor, X: Tensor, *, sv_floor: float = 1e-10) -> LowRankBasis:
    """
    PORT IS DONE!
    fastlmm's low-rank spectral decomposition (lmm_cov.py setSU_fromG lines 138-162)
    Regress X out of the kinship factor G with an OLS hat, decompose the N-by-k result, keep the nonzero
    singular values
    fastlmm SVDs the N-by-k PxG directly.  Going trough eigh of the tiny k-by-k Gᵀ G insted (lmm.py:197 takes
    the same route) -- it gives the same U and s up to float noise but the workspace is k-by-k not N-by-k,
    wich is what keeps the basis affordable at N = 100k
    The k < N branch is the whole point and the case extreme always lands in (capped pruned GRM).  k >= N
    flips to eighing the N-by-N kernel directly, since then k-by-k is the bigger of the two -- that's
    fastlmm's own setSU_fromG switch (lmm_cov.py:141), kept here as a robustness fallback for the corner where
    the GRM rank reaches N
    G must be column-standardised (use io.standardise_columns).  G is the pruned kinship markers, separate
    from the test SNPs -- that separation is what caps the rank
    """
    N, k_in = G.shape
    D = X.shape[1]
...
def rotate_phenos(basis: LowRankBasis, Y: Tensor) -> tuple[Tensor, Tensor]:
    """
    PORT IS DONE!
    Rotate a pheno chunk onto an exsiting basis (lmm_cov.py rotate lines 182-211)
    OLS regress the covariates out, split into the on-rank rotation UY and the off-rank residual UUY.  Cheap
    relative to the basis eigh, so it's the part that reruns per pheno chunk
    Returns (UY (k, P), UUY (N, P)).  UUY is the exact residual fastlmm keeps to dodge cancelation, fine to
    materialise here since a pheno chunk is sized to fit
    """
...
def lowrank_rotate(G: Tensor, X: Tensor, Y: Tensor, *, sv_floor: float = 1e-10) -> LowRankSpectrum:
    """
    Basis decompositon + one pheno rotation in one call, the resident-matrix convenience that lowrank_basis +
    rotate_phenos compose
    """
...
def _profile_loss_lowrank(log_delta: Tensor, s: Tensor, UY: Tensor, yy_off: Tensor, Neff: int) -> Tensor:
    """
    PORT IS DONE!
    Vectorised -2*loglik on a (G grid x P pheno) tensor for the low-rank path (lmm_cov.py nLLcore +
    computeAKA, ML branch)
    The quadratic form splits in two: the on-rank part weights UY by 1/(s + delta) over the k kept dims, the
    off-rank part is the residal SS yy_off shared across the Neff - k null dims, each seeing denom = delta.
    log det picks up (Neff - k) log delta for those null dims
    Neff = N - D is the reduced sample size, not the rank k -- that is the whole diference from the full-rank
    compat loss.  Returns shape (G, P)
    """
    k = s.shape[0]
...
def _profile_loss_lowrank_per_pheno(log_delta: Tensor, s: Tensor, UY: Tensor, yy_off: Tensor,
                                    Neff: int) -> Tensor:
    """
...
def fit_delta_grid_lowrank(spec: LowRankSpectrum, *, n_grid: int = 64, log_delta_min: float = -10.0,
    ...
    Same search as fit_delta_grid_compat on the low-rank profile loss.  ML only, fastlmm's findH2 has no REML
    branch
    """
    s, UY = spec.s, spec.UY
    ...
    # vectorised golden-section refinment on [grid[idx-1], grid[idx+1]] per pheno, 50 iters lands past what
    # the scan needs
...
def _auto_tile_lowrank(N: int, k: int, P: int, M: int, device: torch.device, elem_size: int) -> tuple[int,
                                                                                                      int]:
    """
    Pick pheno / snp tile sizes for the low-rank scan from the GPU memory free right now
    The heavy live tensors per (pheno tile Pc, snp tile Mc) are the two N-by-Mc slabs S_r and UUS, the N-by-Pc
    off-rank UUY, and the dozen-ish (Pc, Mc) stat temporaries.  budgeting half the free vram for that leaves
    slack for the cholesky-free inner and allocator fragmentaion
...
def snp_wald_scan_lowrank(spec: LowRankSpectrum, log_delta: Tensor, S: Tensor, *,
                          snp_chunk: int | None = None, pheno_chunk: int | None = None,
                          n_real: int | None = None) -> ScanResult:
    """
    PORT IS DONE!
    Multi-pheno per-(SNP, pheno) Wald F-stat in the low-rank basis (lmm_cov.py nLLcore + computeAKA /
    computeAKB, the per-SNP loop in single_snp.py around line 1300)
    Test SNPs get the same OLS regress-out and split into on-rank US and off-rank UUS the phenos took in
    lowrank_rotate.  Every quadratic form carrys both parts: the on-rank weighted by 1/(s + delta), the
    off-rank by 1/delta
    Per-(SNP, pheno) dof is Neff - 1 = N - D - 1, the F-test in single_snp.py:1415.  sigma2 divides by Neff,
    not the rank k
    n_real splits the pheno axis the usual way: leading columns keep full per-SNP detail, the rest are perm
    colums feeding only the per-column running max F.  Returns a ScanResult
    """
...
def loco_scan_lowrank(G: Tensor, g_chrom: np.ndarray, Z: Tensor, z_chrom: np.ndarray, X: Tensor, Y: Tensor, *,
                      n_real: int | None = None, on_chrom = None) -> ScanResult:
    """
    Multi-pheno Leave-One-Chromosome-Out scan in the low-rank basis
    G is the pruned kinship factor with its own per-column chromosome labels g_chrom, Z is the test SNP matrix
    with labels z_chrom -- two seperate matrices, unlike the single-Z compat path, since the cap on the
    kinship rank lives in G
    For each chromosome c the kinship factor drops c's pruned markers, refit per-pheno log delta against the
    c-out basis, scan Z's c-only SNPs.  Each chromosone's SNPs carry that chromosome's null h2.  Both G and Z
    must be column-standardised
    """
    M = Z.shape[1]
    P = Y.shape[1]
    if n_real is None:
...
def single_k_scan_lowrank(G: Tensor, Z: Tensor, X: Tensor, Y: Tensor, *,
                          n_real: int | None = None) -> ScanResult:
    """
    Non-LOCO whole-genome scan in the low-rank basis: one kinship factor G, one rotation + delta-fit, one scan
    over all of Z
    Same proximal-contaminaton caveat as core.single_k_scan_compat, just the low-rank rotation insted of
    forming K.  G and Z both column-standardised
    """
