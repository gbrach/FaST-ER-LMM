...
Keeping it minimal here, no schema.  The watcher just reads whatever keys are in the payload and decides what
to display
"""
...
def write_status(path: str | Path, payload: dict) -> None:
    ...
    Atomicity matters becuase a watcher could be reading at the same momment and a half-written json crashes
    the parser.
...
def pbar(iterable: Iterable[T], *, desc: str = "", total: int | None = None,
         leave: bool = True) -> Iterator[T]:
    """
    tqdm wrapper, falls back to a no-op iterator if tqdm is not installed
    The watcher reads status.json for richer infos anyway, this is just so the terminal isn't completely dead
