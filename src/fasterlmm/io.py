...
Not a port of anything in fastlmm: their loading goes through _snps_fixup and _pheno_fixup in
fastlmm_predictor.py, which accept strings or already-built SnpReader/KernelReader objects.
...
def read_plink(prefix: str | Path, *, count_A1: bool = True) -> Genotypes:
    ...
    The mean-impute-then-zero-NaN pattern from pysnptools' Unit standardizer goes in a separate helper later,
    not pulling it in while there's nothing downstream
    """
    from pysnptools.snpreader import Bed

    bed = Bed(str(prefix), count_A1 = count_A1).read()
    return Genotypes(Z = np.asarray(bed.val, dtype = np.float64), iid = [str(x[1]) for x in bed.iid],
                     sid = [str(x) for x in bed.sid], chrom = np.asarray(bed.pos[:, 0]),
...
class Covariates:
    """
    plink-style covariate table.  C (N, K) is the covariate matrx, names are the column labels (cov_0, cov_1,
    ... since plink .cov has no header)
    """
    iid: list[str]
    names: list[str]
    C: np.ndarray
...
class AlignedDataset:
    ...
    iid is the N strain order, Z (N, M) the genotypes, Y (N, P) the phenos, X (N, C) is [intercept |
    covariates], chrom/pos/snp_id describe the M columns of Z, pheno_names labels the P columns of Y
    """
    iid: list[str]
    Z: Tensor
    Y: Tensor
...
def standardise_columns(arr: np.ndarray | Tensor) -> np.ndarray | Tensor:
    ...
    Numpy and torch branches both kept because Z lands either way dependig on whether the caller has handed me
    a raw bed.val or a torch tensor already
    """
...
def grm(Z: Tensor) -> Tensor:
    """
    K = Z Zᵀ / M.  Z must already be standardised (mean 0, std 1) per column, otherwise the eigenvalues come
    out scaled in nonsense units
    Same definition as fastlmm's SnpKernel (pysnptools/kernelreader/snpkernel.py _read), no need of rebuilding
    it from scrach
    """
    M = Z.shape[1]
    return (Z @ Z.T) / M
