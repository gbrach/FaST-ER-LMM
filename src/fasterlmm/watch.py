...
Point it at the outdir and it builds the multi-GPU view: one overall panel (aggregate progress, rate, ETA,
Wald ops, RAM) plus one panel per shard (per-GPU progress bar, rate, the live loco sweep, GPU memory).  Point
it at a single status.json and it shows the one-pane view
The watcher and the runner are decoupled, the runner drops json status snapshots and the watcher just polls
them, so a shared filesystem is all it needs
usage:
...
def _read_payload(path: Path) -> dict | None:
    """
    returning None on a missing file or a mid-rename half-write, the caller decides what to render in the
    placeholer
    """
    try:
...
def _discover_shards(outdir: Path) -> dict[int, dict]:
    """
    Globbing status.shard*.json in outdir, skipping the .tmp atomic-rename siblings that flash into existance
    for a millisecond during write_status
...
def _all_done(parent: dict | None, shards: dict[int, dict]) -> bool:
    """
    Flagging done when the parent status.json says done, OR when every discovered shard says done and at least
    one shard exists
    Parent-side covers auto-dispatch (the parent writes the finalizer after the spawn join), shard-side covers
    an explicit --shard slurm-array where noone writes the parent
    """
...
def _aggregate(states: list[dict[str, Any]]) -> dict[str, Any]:
    """
    rolling the per-shard snapshots up into one view. phenos sum across shards, elapsed is the slowest shard,
    rate is real phenos over wall time
    """
    ...
    return {"phenos_done": done, "phenos_total": total, "elapsed_s": elapsed, "rate": rate, "eta_s": eta,
...
def _build_overall_panel(agg: dict[str, Any], n_shards: int, run_name: str, width: int) -> Panel:
    ...
    # Wald ops: every (variant, pheno-col) pair is one beta/SE solve, times (1 + n_perm) cols. the number
    # lands in the trillions, feels-fast in a way pheno/s doesn't
    ...
    _kv("writes", f"{int(wp):,} pending" if wp else "-", PALETTE["warn"] if wp else PALETTE["muted"])
    body.append("\n")
    ...
    return Panel(body, title = title, border_style = PALETTE["primary"], padding = (1, 2), title_align = "left")


_STATE_GLYPH: dict[str, tuple[str, str]] = {"loading": ("·", "muted"), "scanning": ("▶", "warn"),
                                            "writing": ("▸", "warn"), "dry-run": ("·", "muted"),
                                            "done": ("✓", "success")}


def _build_shard_panel(idx: int, s: dict[str, Any], n_shards: int, panel_width: int) -> Panel:
    ...
    if chroms_total:
        ...
        chroms_done = (min(int(s.get("chroms_done", 0)),
                           chroms_total) if state == "scanning" else chroms_total)
    ...
    body.append(_fmt_eta(((total - done) / rate) if (rate and total > done) else None), style = PALETTE["warn"])
    ...
    if cpu_frac is not None:
        ...
        body.append(f"cpu {cpu_frac * 100:.0f}%", style = PALETTE["warn"] if io_bound else PALETTE["success"])
    ...
    if g_alloc is not None or g_total is not None:
        ...
        if g_total and g_alloc:
            ...
            col = (PALETTE["fail"] if pct > 90 else PALETTE["warn"] if pct > 70 else PALETTE["success"])
    ...
    return Panel(body, title = title, border_style = color, padding = (0, 1), title_align = "left")


def _render(parent: dict | None, shards: dict[int, dict], run_name: str, width: int) -> Group | Panel | Text:
    ...
    if shards:
        ...
        shard_view = (panels[0] if n == 1 else Columns(panels, expand = True,
                                                       equal = True) if side_by_side else Group(*panels))
        return Group(overall, shard_view)

    # no shard files: either a single-GPU run (parent IS the worker) or still dispatching
    if parent and ("phenos_total" in parent or parent.get("state") in ("loading", "scanning", "done")):
    ...
    if parent and parent.get("state") == "dispatch":
        return Panel(Text("workers spawning, waiting for the first shard snapshot…", style = PALETTE["warn"]),
...
def main() -> None:
    ...
    A directory (or a not-yet-existing path whose parent already holds shard files) is the multi-GPU
    dashboard, a plain status.json is the single pane
    """
    parser = argparse.ArgumentParser(prog = "fasterlmm watch", description = "live TUI dashboard for a running "
                                     "fasterlmm gwas job, single or multi-GPU")
    parser.add_argument("path", help = "outdir (multi-GPU dashboard) or a status.json file (single pane)")
    args = parser.parse_args()

    path = Path(args.path)
    # multi mode if path is a directory, or if it doesn't exist yet but its parent already has shard files
    # (watcher launched before the parent finalizer wrote status.json)
    if path.is_dir():
