...
The exact low-rank engine (lowrank + extreme_scan) for the case where N gets large enough that forming the
N-by-N kernel and holding the whole N-by-M genotype both fall over.  the kinship comes from a capped pruned
marker set (auto-strided, or a --grm bed), the test variants stream past it a block at a time, phenos go in
chunks that re-stream the genotype per chunk
Builds on the frozen core -- it reuses the gwas writer and the bundle dataset as-is so the output schema is
identical, only the scan path underneath is the streamed low-rank one.  single device for now, the multi-gpu /
multinode dispatch is the adaptive layer on top
"""
...
def _pack_perms(Y_real: torch.Tensor, pheno_idx: list[int], n_perm: int, seed: int) -> torch.Tensor:
    """
    Pack the B real phenos and their B * n_perm row-shuffles into one (N, B + B*n_perm) tensor
    Same independant per-pheno permutation scheme as perms.perm_threshold -- pheno b's shuffles are seeded on
    its own colum index so they're stable regardless of how the phenos get chunked.  kept as local code so the
    parity-locked perms.py stays frozen
    """
    N, B = Y_real.shape
...
def _meminfo_available_bytes() -> int | None:
    ...
    The kernel's own estimate of what a fresh alocation can claim without swapping.  this is the NODE's free
    ram though, not what a slurm cgroup actualy lets the job touch, so _ram_headroom_bytes caps it with the
    cgroup limit
    """
    try:
...
def _cgroup_headroom_bytes() -> int | None:
    """
    Whats left of this job's cgroup memory limit (limit - current usage), v2 then v1, or None when uncapped /
    unreadable
    Slurm caps a --mem job trough a cgroup, and /proc/meminfo dosn't see that cap -- so without this the probe
    reads the whole node's free ram and happily over-commits a 40GB slab into a job that only got 16GB, wich
    the oom killer then ends.  a "max" limit means uncapped
    """
...
def _want_resident(mode: str, need_bytes: int) -> bool:
    ...
    auto probes the free ram (cgroup-capped) and goes resident only when the N x M slab leaves room to spare
    for the working set + perms + writers (half the headroom, a deliberately roomy margin).  on forces it, off
    always streams -- the streaming fallback is what keeps the path alive past the point N x M stops fiting,
    the march toward a million strains
    """
    if mode == "off":
        return False
    if mode == "on":
...
def _run_extreme(args: argparse.Namespace, shard_i: int | None, shard_n: int | None, device: str) -> None:
    """
    Load inputs, build the pruned kinship factor, stream the LOCO scan per pheno chunk, write the bundle

    shard_i / shard_n slice the pheno list round-robin so concurent workers (one per GPU, or a slurm-array
    task) split the phenos evenly.  status file is per-shard so the workers dont stomp each other
    """
    ...
    # dosn't depend on the phenos), so when the standardised N x M slab fits ram we decode it once here and
    # slice
    # it per batch insted of re-reading the bed every time.  past the point it stops fiting we fall back to
    # streaming
    ...
    # round-robin slice so each shard carries a balanced spread of phenos, display_offset tiles the printed
    # ranges
    display_offset = 0
    ...
    writer_ctx = {"snp_id": pa.array(handle.sid, type = pa.string()), "chrom": handle.chrom,
    ...
    try:
        for b_start in range(0, len(pheno_list), args.phenos_per_job):
            ...
            for j, p in enumerate(batch):
                write_futures.append(writer_pool.submit(_write_pheno, writer_ctx, str(outdir), pheno.names[p],
                                                        p_col = p_real[:, j], beta_col = beta_np[:, j],
                                                        se_col = se_np[:, j], sfve_col = sfve_np[:, j],
            ...
            print(f"{log_prefix}batch {display_offset + b_start}..{display_offset + b_start + B}: "
                  f"{B} phenos in {time.time() - t_batch:.1f}s", file = sys.stderr, flush = True)
            write_status(status_file,
...
def _shard_entrypoint(rank: int, n_gpu: int, args_dict: dict) -> None:
    ...
    Pins the process to its own device with CUDA_VISIBLE_DEVICES before any cuda call so the worker sees one
    GPU as cuda:0 -- seperate processes each get their own cuda context, wich also sidesteps the eigh
    lazy-init thread race.  args arrives as a dict so the parent never imports torch before the children set
    the env var
    """
...
def main() -> None:
    ...
    """
    parser = argparse.ArgumentParser(prog = "fasterlmm extreme", description = "big-N LOCO GWAS, exact low-rank "
                                     "scan with a streamed genotype and a capped kinship")
    ...
    parser.add_argument("--grm", default = None, help = "plink BED prefix for a pre-pruned kinship marker set "
                        "(optional, else auto-strided from --geno)")
    parser.add_argument("--grm-k", type = int, default = 5000, help = "target kinship marker count for the "
                        "auto-stride when --grm is not given (default 5000)")
    ...
    parser.add_argument("--phenos-per-job", type = int, default = 64, help = "real phenos packed into one "
                        "streamed scan, the genotype re-streams once per chunk. gpu cols = this x (1 + "
                        "n_perm)")
    parser.add_argument("--block-size", type = int, default = 8192, help = "test variants held resident per "
                        "streamed block, the lever that keeps the N x M genotype off the heap")
    parser.add_argument("--resident", choices = ["auto", "on", "off"], default = "auto",
                        help = "hold the standardised genotype resident when N x M fits ram (auto, the "
                        "default), force on, or always stream (off). resident decodes once insted of "
                        "re-streaming every pheno batch")
    parser.add_argument("--write-workers", type = int, default = None,
                        help = "writer threads, unset fills the " "core allocation")
    ...
    parser.add_argument("--float64", action = "store_true",
                        help = "run in float64 (default float32, the scale " "setting)")
    ...
    parser.add_argument("--bundle", action = "store_true",
                        help = "stream the per-pheno tables into one " "gwas_bundle.parquet")
    ...
    # point them at the live watcher -- it takes the outdir and autodiscovers single-pane vs the per-shard
    # dashboard
    ...
    # fold the per-shard bundle parts into the final dataset, same as the gwas path (a --shard array uses
    # fasterlmm concat)
    if args.shard is None:
        ...
        # merge over the rich done payload the single-device worker already wrote (N, M, phenos, peak RAM,
        # elapsed)
