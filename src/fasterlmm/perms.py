...
The empirical 5% quantile of those min p-values is the genome-wide significance threshold under "no
association".
A batch of real phenos plus all their permutation columns ride one scan together, so the per-chromosome
eigendecomposition is paid once for the whole batch rather than once per pheno -- that is where the speed
comes from
"""
...
def perm_threshold(data: AlignedDataset, pheno_idx: list[int], *, n_perm: int = 100, seed: int = 19930909,
                   loco: bool = True, on_chrom = None) -> tuple[ScanResult, Tensor]:
    """
    Min-F permutation null distribution for a whole batch of phenos in one scan
    Pack the B real phenos and their B * n_perm permutation columns into one (N, B + B*n_perm) matrix, run
    loco_scan_compat once, slice the per-pheno results back out.
    The eigendecomposition is genotype-only so it costs the same for one column or fifteen thousand --
    batching is what amortizes it across phenos, not just across perms.
    Each pheno gets its own n_perm independent row-shuffles, the way fastlmm does it -- the rng is seeded per
    (seed, pheno index) so a pheno sees the same perms no matter which batch it lands in or how phenos-per-job
    is set.
    Returns (ScanResult for the B real phenos, perm_max_F (B, n_perm)).  perm_max_F is the per-perm genome max
    F, monotone-inverse to the per-perm min p.
