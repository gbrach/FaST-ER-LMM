...
from fasterlmm.core import (fastlmm_compat_rotate, fit_delta_grid_compat, snp_wald_scan_compat)
from fasterlmm.io import grm, standardise_columns
from fasterlmm.lowrank import (fit_delta_grid_lowrank, loco_scan_lowrank, lowrank_basis, lowrank_rotate,
                               rotate_phenos, single_k_scan_lowrank, snp_wald_scan_lowrank)
...
def test_lowrank_basis_k_ge_N_branch():
    ...
    G = torch.tensor(standardise_columns(rng.binomial(2, 0.3, size = (Nw, Mw)).astype(np.float64)), dtype = DTYPE)
    X = torch.tensor(np.concatenate([np.ones((Nw, 1)), rng.standard_normal((Nw, Dcov))], axis = 1), dtype = DTYPE)
    basis = lowrank_basis(G, X)
    D = X.shape[1]
