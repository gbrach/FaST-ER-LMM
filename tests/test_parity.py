...
def _run_gwas(geno: Path, pheno: Path, outdir: Path, covar: Path | None) -> None:
    ...
    argv = [sys.executable, "-m", "fasterlmm", "gwas", "--geno", str(geno), "--pheno", str(pheno), "--outdir",
            str(outdir), "--device", "cpu", "--no-rint", "--n-perm", "1", "--bundle"]
...
@pytest.fixture(scope = "module")
def with_covar_run(parity_geno: Path, parity_pheno: Path, parity_covar: Path,
                   tmp_path_factory: pytest.TempPathFactory) -> pd.DataFrame:
...
def test_parity_no_covar(pheno: str, no_covar_run: pd.DataFrame, parity_golden_dir: Path) -> None:
...
@pytest.mark.parametrize("pheno", PHENOS)
def test_parity_with_covar(pheno: str, with_covar_run: pd.DataFrame, parity_golden_dir: Path) -> None:
