...
from fasterlmm.io import (align_inputs, grm, read_covar, read_phen, read_plink, standardise_columns)
...
def test_standardise_columns_nan_zeroed_after():
    ...
    a = np.array([[1.0, 10.0], [np.nan, 20.0], [3.0, np.nan], [5.0, 40.0], [7.0, 50.0]])
    out = standardise_columns(a)
    assert out[1, 0] == 0.0
    assert out[2, 1] == 0.0
...
def test_standardise_columns_constant_column_zeros():
    ...
    a = np.array([[2.0, 1.0], [2.0, 4.0], [2.0, 9.0], [2.0, 16.0]])
    out = standardise_columns(a)
...
def test_align_inputs_intersection_and_reorder():
    ...
    g = Genotypes(Z = Z, iid = ["A", "B", "C"], sid = ["s0", "s1"], chrom = np.array([1, 1]), pos = np.array([10, 20]))
...
def test_align_inputs_no_overlap_raises():
    ...
    g = Genotypes(Z = np.zeros((2, 1)), iid = ["A", "B"], sid = ["s0"], chrom = np.array([1]), pos = np.array([1]))
